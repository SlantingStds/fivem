...
struct CEntityScriptInfoDataNode : GenericSerializeDataNode<CEntityScriptInfoDataNode>
{
	...
	bool Serialize(Serializer& s)
	{
		...
		if (hasScript) // Has script info
		{
			...
			//console::DPrintf("net", "m_scriptHash %u\n", m_scriptHash);
			// ---> end

			s.Serialize(32, m_timestamp);
			//console::DPrintf("net", "m_timestamp %u\n", m_timestamp);

			uint32_t positionHash = 0;

			bool hasPositionHash = positionHash != 0;
			s.Serialize(hasPositionHash);
			//console::DPrintf("net", "hasPositionHash %s\n", hasPositionHash ? true : false);

			if (hasPositionHash)
			{
				s.Serialize(32, positionHash);
				//console::DPrintf("net", "positionHash %u\n", positionHash);
			}

			int32_t instanceId = -1;
			bool hasInstanceId = instanceId != -1;
			s.Serialize(hasInstanceId);
			//console::DPrintf("net", "hasInstanceId %s\n", hasInstanceId ? true : false);

			if (hasInstanceId)
			{
				bool flag = false;
				s.Serialize(flag);
				//console::DPrintf("net", "flag %s\n", flag ? true : false);

				if (flag)
				{
					uint16_t v6 = 0;
					if (instanceId - 1 <= 0x1F3Fu)
					{
						//v6 = rage::CNetIdMapManager::sm_Instance.m_objectIds1->m_objectIds[this->m_objectId];
					}
					s.Serialize(13, v6); /* _writeWord */
				}
				else
				{
					bool longInstanceId = instanceId >= 512;
					s.Serialize(longInstanceId);
					//console::DPrintf("net", "longInstanceId %s\n", longInstanceId ? true : false);

					bool v9 = longInstanceId ? 32 : 9;
					s.Serialize(v9, instanceId);
					//console::DPrintf("net", "instanceId %d\n", instanceId);
				}
			}
			...
			//console::DPrintf("net", "m_scriptObjectId %u\n", m_scriptObjectId);

			uint16_t hostToken = 0;

			bool longHostToken = hostToken >= (1 << 3);
			s.Serialize(longHostToken);
			//console::DPrintf("net", "longHostToken %s\n", longHostToken ? "true" : "false");

			auto hostTokenLength = longHostToken ? 16 : 3;
			s.Serialize(hostTokenLength, hostToken);
			//console::DPrintf("net", "hostToken %u\n", hostToken);

			// end

			bool unkBool3 = false;
			s.Serialize(unkBool3);
			//console::DPrintf("net", "unkBool3 %s\n", unkBool3 ? "true" : "false");
...
struct CPedVehicleDataNode : GenericSerializeDataNode<CPedVehicleDataNode>
{
	...
	bool Serialize(Serializer& s)
	{
		...
		//console::DPrintf("net", "inVehicle %s\n", inVehicle ? true : false);
		//console::DPrintf("net", "vehicleId %u\n", vehicleId);

		bool wasOnHorse = horseId != 0;
		s.Serialize(wasOnHorse);

		if (wasOnHorse)
		{
			s.Serialize(onHorse);
			s.Serialize(13, horseId);
		}
		else
		{
			horseId = 0;
		...
		//console::DPrintf("net", "onHorse %s\n", onHorse ? true : false);
		//console::DPrintf("net", "horseId %u\n", horseId);

		if (inVehicle || onHorse)
		{
			s.Serialize(5, seat);
		}
		else
